...
const ajvCompilerObj = (
  ajv: ajv.Ajv,
  routeValidators: IRouteValidators
): IAjvCompilerObj => ({
  request: routeValidators.request
    ? ajv.compile(routeValidators.request)
    : null,
  response: routeValidators.response
    ? ajv.compile(routeValidators.response)
    : null,
  ...
  Object.keys(schemas).forEach((controller: string) => {
    ...
    [...Object.keys(schemas[controller])].forEach((method: string) => {
      ...
      if (level === 2) {
        ...
        schemaValidators[controller][method] = ajvCompilerObj(
          ajv,
          routeValidators
        );
        return;
      }
      [...Object.keys(schemas[controller][method])].forEach(
        (action: string) => {
          const routeValidators: IRouteValidators =
            schemas[controller][method][action];
          // @ts-ignore
          schemaValidators[controller][method][action] = ajvCompilerObj(
            ajv,
            routeValidators
          );
        }
